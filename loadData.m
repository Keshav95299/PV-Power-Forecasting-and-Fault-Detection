function [pv, weather, forecast] = loadData()
    ...
    pv = readtable(fullfile('data','photovoltaic_measurement_history.csv'));
    weather = readtable(fullfile('data','weather_station_measurement_history.csv'));
    forecast = readtable(fullfile('data','weather_prediction_history.csv'));
